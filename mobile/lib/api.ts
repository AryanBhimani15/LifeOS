...
/**
 * Answers a known question straight from the database — no AI, no quota.
 *
 * The answers were never generated by a model: the server computes them with
 * SQL and the AI only ever picked which question was meant. A button does that
 * for free, so the common questions never depend on a language model being up.
 */
export const askQuery = (kind: QueryKind) =>
  request<QueryAnswer>("/api/queries", { method: "POST", body: { kind } });
